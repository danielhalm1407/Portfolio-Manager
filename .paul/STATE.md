...
Also: `theme.py` split into `apply_figure_theme` (ink + grid, build time) and
`apply_export_theme` (adds the opaque surface, write time), so inline figures carry the repo's
ink instead of plotly's default navy. Verified: the inline figure now reports `#e0e0e0` on
`layout.font` and every tick font, with `yaxis_tickformat` and the secondary-axis title
surviving the merge; the exported HTML is unchanged.
...
| 2026-08-31 | `theme.apply_export_theme` splits: `apply_figure_theme` (ink + grid, called at BUILD time) and the background (still export-only) | The two halves were never the same kind of thing. `#e0e0e0` text is correct in every context — interactive window, Dash page, saved file — while the BACKGROUND is the only genuinely destination-dependent property, since an inline figure sits on a surface the host supplies and a saved file has no host. Bundling them left a builder two bad options: theme nothing and render plotly's default navy text on a dark editor background (which is what `option_probe_figures.py` was doing), or bake in an opaque background and break the inline case. `apply_export_theme` still applies both halves and is idempotent, so `rebalance_study.py:440` is unchanged |
