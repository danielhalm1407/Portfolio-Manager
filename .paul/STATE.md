...
Also: the theme consolidated and the export rewritten — see the five decision rows below.
Suite now 96 passed (was 79) on the 17 new option-pricing tests. Prior note: the palette consolidated. `panel.py:931`/`:559` and `rebalance_study.py:104` now read
`theme.INK` instead of hand-copied `#e0e0e0` literals, leaving one such literal in the repo — in
theme.py. The probe figures take ink and grid through their plotly ARGUMENTS (`_AXIS`,
`_LAYOUT`), so they no longer fall back to plotly's default navy text inline. Verified:
`layout.font`, both axes and the secondary spot axis all report `#e0e0e0`; `PanelBuilder`
`OFF_WHITE`/`dark_axis_style`/`apply_dark_theme` all still resolve to the same value.
...
| 2026-08-31 | The three hardcoded `#e0e0e0` sites now read `theme.INK`; the probe figures take the palette through their plotly ARGUMENTS, not a new helper | `theme.py`'s docstring says it exists because off-white was spelled four ways across `dash_timeseries_app.py`, `panel.py` and the pipelines — but defining the token was only half the job, and `panel.py:931` (`OFF_WHITE`), `panel.py:559` and `rebalance_study.py:104` went on carrying hand-copied literals. All three now read the token, leaving ONE `#e0e0e0` in the repo, in theme.py where it belongs. A short-lived `apply_figure_theme` split was written and then REVERTED: it was a third implementation of "make a figure dark" beside `PanelBuilder.apply_dark_theme` and `make_level_figure`, which is the very duplication theme.py exists to end. `make_level_figure`'s pure-`white` default (`dash_timeseries_app.py:1542`, :1701) is left alone deliberately — whether the Dash surface wants brighter text is a design question, not a stray literal |
| 2026-08-31 | The four probe figures cannot route through a `panel.py` builder, and a NEW generic builder would be needed | Checked rather than assumed. `PanelBuilder.apply_dark_theme` (:1059) is real but is called by only THREE of panel.py's six builders — `add_pca_waterfall` (:619), `add_var_explained_chart` (:698), `add_loadings_comparison` (:781); the three sector builders set colours inline (`:575`, `:668`, `:694`). More decisively, no panel.py builder produces these SHAPES: they are a sector-per-subplot grid, a correlation heatmap and three PCA charts, none of which takes "arbitrary series, one trace per column". `make_level_figure` (`dash_timeseries_app.py:1517`) is the closest generic multi-series builder and still cannot take them — `yaxis2`/`secondary_y` appear nowhere in that module and two of these figures put spot on a right-hand axis, and it is time-series specific while `fig_smirk`'s x-axis is moneyness. One generic multi-series builder serving Dash, the static export and research is 10-01 / 16-07 work |
| 2026-08-31 | `PanelBuilder.apply_dark_theme` / `dark_axis_style` are LIFTED into `theme.py`; the class keeps thin wrappers | They were never panel concepts — they are the theme — but living as classmethods on an 1,100-line class that also loads CSVs meant only that class could reuse them, so pipelines and research scripts each grew a partial copy instead. Now one implementation, three call sites. `PanelBuilder` keeps both methods as wrappers so every existing caller is untouched: it holds the play/animate button (animation machinery, not palette) and the 700x1200 defaults (right for a Dash panel, wrong for a static page — the shared default is None). Verified identical output for the panel path: 700x1200, x-grid off / y-grid on, transparent, ink, secondary-axis merge |
| 2026-08-31 | `theme.GRID` moves from `"#526070"` to `"rgba(255,255,255,0.1)"` | Two grid colours were in use and lifting `dark_axis_style` forced a choice. The rgba wins on two grounds: it is what the panel figures have actually been rendering with, and a TRANSPARENT grid composites correctly against any surface — the same figure now sits on the VS Code editor background, the Dash page div and a static page, and a solid tone that suits one is wrong on the others. Consequence, accepted: figures exported through `apply_export_theme` (the `rebalance_study` scenarios) now carry a slightly fainter grid. `ZEROLINE` stays solid `#526070` — a zero line is a reference value, not a background rule |
| 2026-08-31 | Two comments in `apply_dark_theme` described the OPPOSITE of the code and were corrected on the move, not carried across | Both said "grid lines on x-axes only". The code has always done `x_axis_style = {**axis_style, 'showgrid': False}` and left the y grid on — i.e. HORIZONTAL rules only, which is the right default for a time series since you read a level off the y-axis. Recorded because comment retention is mandatory here and this is the one case where retention means CORRECTING rather than copying |
| 2026-08-31 | Static figure pages are written by `write_figure_page`, not `fig.write_html`; the figure stays transparent and the PAGE carries the dark surface | `write_html` emits a document whose only CSS is `html, body {height: 100%}` — no background, no `margin: 0` — and takes no CSS hook (checked its full parameter list). So the old route left a dark figure inside the browser's default ~8px WHITE body margin. Writing the shell here (`to_html(full_html=False)` plus eleven lines setting the body to `theme.PAGE_BG`) makes the static page behave exactly as the Dash page already does, and means the published figure is the SAME OBJECT as the inline one with nothing stamped on at export. `to_html` only writes the plotly.min.js REFERENCE, never the file, so `main()` writes it once via `plotly.offline.get_plotlyjs()` |
| 2026-08-31 | `tests/test_option_pricing.py` added — 17 tests, the first coverage the option code has had | The write-up claimed the port was "verified" on the strength of a print statement in a research cell, which decays the moment anyone edits a constant. Pins 12-01's four anchors (IV 0.2098 / 0.1760, put 3.612 / 3.206) AND the properties its economics rest on: monotonicity in strike with an explicit strike-SWAP case (catches a spot/strike transposition that a single-strike test sails past), the written leg of a spread carrying more vol than the bought leg, a spread costing strictly between zero and the naked put, the discounted-intrinsic floor across a swept grid, deep-ITM sitting strictly below `K - S` (i.e. NOT clamped), and the degenerate `tau=0` / `vol=0` branches |
