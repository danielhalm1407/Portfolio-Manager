...
Plan: 11-01 created 2026-08-24, awaiting approval — now also carries the `PanelBuilder` ragged-history
fix. Also open: 02-02 and 02-03 (created 2026-08-23, awaiting approval), 04-05 (CONTEXT.md written,
...
Last activity: 2026-08-31 — Roadmap amended, no code touched. Phase 10 split into Track A (10-01,
...
| 2026-08-31 | Current `ffill` + `dropna` stays the DEFAULT; ragged loading is opt-in, for prices and returns alike | Keeping the default preserves every existing caller and figure unchanged, so the fix cannot regress current output. The opt-in mode must cover the derived-frame wrappers (`panel.py:187-211`) too: a rebase or first-difference on a ragged frame anchors to each series' own first observation, not the panel's |
