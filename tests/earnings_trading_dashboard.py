...
class IBApp(EWrapper, EClient):
    """
    IB API bridge class. Inherits from both EWrapper (receives incoming data and events
    from Interactive Brokers) and EClient (sends requests out to IB). This dual-inheritance
    pattern is the standard setup required by the IB Python API.

    Key attributes:
        connected (bool): Set to True once the IB handshake completes via nextValidId().
        historical_data (dict): Stores incoming OHLCV bars keyed by request ID (reqId).
                                Each value is a list of dicts: date, open, high, low, close, volume.

    Key methods:
        error(): Callback fired by IB on errors/warnings. Filters out the noisy
                 fractional-share warning (errorCode 2176).
        nextValidId(): Fired by IB on successful connection. Sets connected = True.
        historicalData(): Callback fired once per bar as historical data arrives.
                          Appends each bar to historical_data[reqId].
        historicalDataEnd(): Fired when a full historical data response is complete.
    """
    def __init__(self):
...
class EarningsTradingDashboard:
    """
    Main application class. Owns the Tkinter window, all UI widgets, the IB connection,
    and the entire analysis pipeline. One class that ties everything together.

    Key attributes:
        stock_data, vix_data, iv_data (pd.DataFrame): Time series pulled from IB for the
            stock price, VIX level, and implied volatility respectively.
        ib_app (IBApp): The IB API bridge instance used to make and receive data requests.
        risk_free_rate (float): Hardcoded at 0.05 (5%), fed into all Black-Scholes calculations.
        ax1_twin: Reference to the secondary y-axis on the left chart, tracked so it can
                  be cleanly removed on reset.

    Key method groups:
        setup_ui(): Builds all Tkinter widgets — connection panel, input fields, metric
                    labels, scrolled log box, and embedded matplotlib canvas.
        connect_ib() / disconnect_ib(): Manage the IB socket connection. connect_ib() spawns
                    a daemon thread for the blocking ib_app.run() call, then polls up to 10s
                    for the handshake to complete.
        analyze_iv_crush(): Entry point for analysis. Pulls three historical data series from
                    IB (stock OHLCV, VIX, implied volatility), each via a separate reqId,
                    then hands off to perform_iv_crush_analysis().
        perform_iv_crush_analysis(): Core logic. Identifies pre/post earnings trading days,
                    prices an ATM straddle using Black-Scholes pre and post earnings, computes
                    IV crush %, straddle P&L for long and short positions, and updates all
                    metric labels in the UI.
        black_scholes_call() / black_scholes_put(): Standard BS formula using scipy.stats.norm.
        calculate_delta() / calculate_vega(): Greeks for the ATM straddle, computed pre and post.
        create_visualizations(): Draws two matplotlib subplots — stock price + IV overlay on
                    the left, VIX line (or straddle price bar chart if no VIX) on the right.
        clear_analysis_results(): Resets all labels to N/A and clears the charts, including
                    safely removing any twin y-axis.
    """
